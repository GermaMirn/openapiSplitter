...
- **Redis** — при росте нагрузки: кэш дерева и списков по path prefix для снижения количества запросов к files-service; при необходимости — очереди для фоновой разрезки больших спецификаций.
- **SSE для загрузки** — при длительных загрузках: Server-Sent Events с прогрессом (парсинг → валидация → разрезка → сохранение в files-service), чтобы фронт показывал индикатор и не зависал на долгих запросах.
